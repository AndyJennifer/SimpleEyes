 ...
 * 1.全局的onNext拦截器[globalOnNextInterceptor]，用于拦截所有的onNext事件
 ...
 *
 * 整个流程分为：
 * 1.判断事件流是否有onError事件，如果没有走全局的onNext拦截器
 * 2.如果事件流中有onError事件，那么会先走，全局的错误处理器，接着走全局的重试处理器，并通过设置的重试配置，控制重试次数。
 *
 */
